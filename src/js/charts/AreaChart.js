...
import { AreaChart as AreaRechart, Area, XAxis, YAxis, Tooltip } from "recharts";
      ...
      <defs>
        {dataKeys.map((key, i) => (
          <linearGradient key={key} id={key} x1="0" y1="0" x2="0" y2="1">
            <stop offset="5%" stopColor={chartColors[i % 4]} stopOpacity={0.8}/>
            <stop offset="95%" stopColor={chartColors[i % 4]} stopOpacity={0}/>
          </linearGradient>
        ))}

      </defs>
        <XAxis scale="time" type="number" axisLine={false} dataKey={xAxisKey} tick={{ fill: "#29434e" }}/>
        <YAxis tick={{ fill: "#29434e" }} />
        <Tooltip animationDuration={500}/>
        {dataKeys.map((key, i) => (
          <Area type='monotone' dataKey={key} stroke={chartColors[i % 4]} fillOpacity={1} fill={`url(#${key})`}/>
        ))}
      </AreaRechart>
    );
  }

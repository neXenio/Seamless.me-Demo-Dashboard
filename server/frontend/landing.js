...
var preselectedDataId = 'com.nexenio.behaviourauthentication.core.internal.behaviour.data.sensor.data.GravitySensorData';
...
function processDataRecordingContainer(partialDataRecordingContainer) {
  ...
  $("#statusText").text('Processing partial data recording with ' + delay + 'ms delay');
...
function onDeviceWithNewIdConnected(device) {
  ...
  var otherConnectedDevices = connectedDevices.filter(connectedDevice => connectedDevice.id != device.id);
...
function updateChartPlot() {
  ...
  for (var dimension = 0; dimension < dimensions; dimension++) {
    var valuesInDimenion = dataRecordingContainer.getDataValuesInDimension(selectedDataId, dimension);
    xValues.push(delays);
...
window.setInterval(render, RENDERING_INTERVAL);
